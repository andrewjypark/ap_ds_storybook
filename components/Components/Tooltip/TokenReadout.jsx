 ...
 * (see Button's/Checkbox's for the full rationale) -- reads live
 * custom-property values off an externally supplied ref, since the
 * "--ap-tooltip-*" vars a swatch cares about are set on the real DOM
 * node itself (see Tooltip.jsx), not on this readout.
 */
...
export function TokenReadout({ targetRef, rows }) {
	...
	return (
		<div className="ap-tooltip-readout">
			{rows.map((row) => (
				<div className="ap-tooltip-readout-row" key={row.cssVar}>
					<span className="ap-tooltip-readout-label">{row.label}</span>
					<div className="ap-tooltip-readout-row-info">
						<span className="ap-tooltip-readout-var">{row.cssVar}</span>
						<span className="ap-tooltip-readout-sep">|</span>
						<span className="ap-tooltip-readout-value">{values[row.cssVar] || "…"}</span>
					</div>
				</div>
			))}
		</div>
